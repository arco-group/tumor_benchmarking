'''
This project uses the official nnU-Net implementation from MIC-DKFZ:
https://github.com/MIC-DKFZ/nnUNet
The evaluation code is adapted from the nnU-Net codebase, but modified to fit our needs. The original code can be found here: https://github.com/MIC-DKFZ/nnUNet/blob/master/nnunetv2/evaluation/evaluate_predictions.py
'''
...
if __name__ == '__main__':
    ...
    parser.add_argument("--nnunet_config", type=str, choices=["2d", "3d_fullres", "3d_lowres"])
    parser.add_argument("--folder_ref", type=str)
    parser.add_argument("--folder_pred", type=str)
    parser.add_argument("--output_file", type=str)
    parser.add_argument("-labels_to_list", default=1, help="labels to list, e.g. 1,2,3.")
    args = parser.parse_args()

    image_reader_writer = SimpleITKIO()
    file_ending = '.nii.gz'
    regions = labels_to_list_of_regions([args.labels_to_list])
    ignore_label = None
    num_processes = 12
    compute_metrics_on_folder(args.folder_ref, args.folder_pred, args.output_file, image_reader_writer, file_ending, regions, ignore_label, num_processes)